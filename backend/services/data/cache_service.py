...
class EnhancedCache:
    ...
    def get_valuation_cache(self, etf_code: str, trade_date: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        return self.get_daily_cache(date_key, "valuation", etf_code)

    def set_valuation_cache(self, etf_code: str, data: Dict[str, Any], trade_date: Optional[str] = None):
        """保存交易日估值缓存"""
        date_key = trade_date or datetime.now().strftime('%Y%m%d')
        self.set_daily_cache(date_key, "valuation", etf_code, data)

    def get_valuation_with_fallback(self, etf_code: str, trade_date: Optional[str] = None) -> Dict[str, Any]:
        """
        获取估值数据（带三级平滑降级支持）
        1. 尝试交易日缓存
        2. 缓存未命中时由调用方在线抓取
        ...
        """
        # 1. 尝试读取日频缓存
        cached = self.get_valuation_cache(etf_code, trade_date)
        if cached:
            return cached

        # 2. 离线底表降级
        baseline = self.get_valuation_baseline()
        ...
        return {
            ...
            'temperature': 50.0,
            'tier': '适温合理',
            'advice': '暂无该标的长期估值统计数据，建议按标准网格运行'
        }
